...
A PostgreSQL client for [Milo](https://github.com/milo-language/milo), speaking
wire protocol v3 directly. No libpq, no C dependency for the protocol itself —
...
fn main(): i32 {
    ...
    for row in rows.all() {
        print((row.i64("id") ?? 0).toString() + " " + (row.str("name") ?? "") + " " + (row.f64("score") ?? 0.0).toString())
    }
